...
function StaffApp() {
  ...
  // THEME COLORS
  // ============================================================
  const bgColor = darkMode ? '#0f0f1a' : '#eff6ff'
  ...
  async function loadUnpaidOrders() {
    try {
      ...
      const confirmed = (data || []).filter(order => {
        const workflowStatus = order.order_status || order.status
        return ['new', ORDER_STATUS.CONFIRMED, ORDER_STATUS.PREPARING, ORDER_STATUS.READY, 'preparing', 'ready', 'served'].includes(workflowStatus) ||
               ['new', ORDER_STATUS.CONFIRMED, ORDER_STATUS.PREPARING, ORDER_STATUS.READY, 'preparing', 'ready', 'served'].includes(order.status)
      })
      ...
      <div style={{ ...glassEffect, background: cardBg, borderRadius: '28px', padding: isMobile ? '20px' : '28px', maxWidth: '720px', width: '100%', maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          <h2 style={{ margin: 0, color: textColor, fontSize: isMobile ? '18px' : '22px' }}>
            🔔 {t('new_orders_title')} ({newOrders.filter(o => !notifiedOrderIds.has(o.id)).length})
          </h2>
        ...
        {newOrders.filter(o => !notifiedOrderIds.has(o.id)).length === 0 ? (
          <div style={{ textAlign: 'center', padding: '40px 20px', color: textMuted }}>📭 {t('no_new_orders')}</div>
        ) : newOrders.filter(o => !notifiedOrderIds.has(o.id)).map(order => (
          ...
          <div style={{
            ...
            <button
              ...
              style={{
                ...
                boxShadow: '0 4px 16px rgba(37,99,235,0.3)',
                position: 'relative',
                animation: newOrders.filter(o => !notifiedOrderIds.has(o.id)).length > 0 ? 'pulse 1s ease-in-out infinite' : 'none'
              }}
            >
              🔔 {t('new_orders_title')}
              {newOrders.filter(o => !notifiedOrderIds.has(o.id)).length > 0 && (
                <span style={{
                  ...
                  {newOrders.filter(o => !notifiedOrderIds.has(o.id)).length}
                </span>
              )}
            </button>

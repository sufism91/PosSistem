...
function CustomerDisplay() {
  ...
  // ============================================================
  // 🔥 TAMBAH: PRINT ALL RECEIPTS
  // ============================================================
  const printAllReceipts = () => {
    if (tableOrders.length === 0) {
      toast.error(t2('no_orders'))
      return
    }
    
    tableOrders.forEach(order => {
      printReceiptWithHook(order)
    })
    
    toast.success(`🖨️ ${tableOrders.length} ${language === 'bm' ? 'resit dicetak!' : 'receipts printed!'}`)
  }
  ...
  return (
    <div style={{ 
      ...
      {/* ===== SPECIAL MENU BANNER - TANPA GAMBAR ===== */}
            ...
            {specialMenuItems.slice(0, isMobile ? 4 : 8).map((item, idx) => (
              ...
              >
                ...
                <div style={{ 
                  ...
                  RM {typeof item.price === 'number' ? item.price.toFixed(2) : Number(item.price).toFixed(2)}
                </div>
              </div>
            ))}
          </div>

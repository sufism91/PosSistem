...
function TableQRs() {
  ...
  const translations = {
    ...
    small: { en: 'Small (110px)', ms: 'Kecil (110px)' },
    medium: { en: 'Medium (150px)', ms: 'Sederhana (150px)' },
    large: { en: 'Large (190px)', ms: 'Besar (190px)' },
    ...
    const generateQRCodes = async () => {
      ...
      for (const table of tables) {
        if (selectedTables.includes(table.id)) {
          ...
          try {
            // Saiz untuk 2 QR dalam 1 meja (4 QR per page)
            const size = printSize === 'large' ? 190 : printSize === 'medium' ? 150 : 110
  ...
  // ============================================================
  // PRINT FUNCTION - 1 KERTAS = 2 MEJA (4 QR)
  // ============================================================
  const handlePrint = () => {
    ...
    const sizeValue = printSize === 'large' ? 190 : printSize === 'medium' ? 150 : 110
    ...
    for (let i = 0; i < selectedTablesList.length; i += tablesPerPage) {
      ...
      for (const table of pageTables) {
        ...
        const qrImg1 = qrDataUrlsArray[0] 
          ...
          : `<div style="width:${sizeValue}px;height:${sizeValue}px;background:#f1f5f9;display:flex;align-items:center;justify-content:center;margin:0 auto;border-radius:6px;color:#94a3b8;font-size:10px;">QR Error</div>`
            ...
            {selectedTablesList.length === 0 ? (
              ...
              <div>
                {(() => {
                  ...
                  const previewSize = Math.min(sizeValue, 130)
